...
export class BlocksService {
  ...
  getBlockType(token: any) {
    ...
    return this.http.get(`${this.API_URI}/global/blockTypes`, {
      headers: headers,
    });
  }
  ...
  getBlock(token: any, id: string) {
    let headers = new HttpHeaders({ 'x-token': token });
    return this.http.get(`${this.API_URI}/block/getOne/${id}`, {
      headers: headers,
    });
  }
  ...
  deleteBlock(token: any, id: string) {
    let headers = new HttpHeaders({ 'x-token': token });
    return this.http.delete(`${this.API_URI}/block/delete/${id}`, {
      headers: headers,
    });
  }
}
