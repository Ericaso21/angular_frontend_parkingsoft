...
export class AcessPermitsService {
  ...
  getModule(token: any) {
    ...
    return this.http.get(`${this.API_URI}/global/modules`, { 'headers': headers });
  }

  getAccessPermits(token: any) {
    let headers = new HttpHeaders({ 'x-token': token });
    return this.http.get(`${this.API_URI}/accessPermits/list`, { 'headers': headers });
  }

  getAcessPermit(token: any, id: string) {
    let headers = new HttpHeaders({ 'x-token': token });
    return this.http.get(`${this.API_URI}/accessPermits/getOne/${id}`, { 'headers': headers });
  }
  ...
  updateAccessPermit(id: string | number, accessPermit: AccessPermits) {
    return this.http.put(`${this.API_URI}/accessPermits/update/${id}`, accessPermit);
  }

  deleteAccessPermit(token: any, id: string) {
    let headers = new HttpHeaders({ 'x-token': token });
    return this.http.delete(`${this.API_URI}/accessPermits/delete/${id}`, { 'headers': headers });
  }
}
