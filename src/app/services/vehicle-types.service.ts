...
export class VehicleTypesService {
  ...
  getVehicleTypes(token:any){
    ...
    return this.http.get(`${this.API_URI}/vehicleTypes/list`,{'headers' : headers})
  }

  getVehicleType(token: any, id: string){
    let headers = new HttpHeaders({ 'x-token': token });
    return this.http.get(`${this.API_URI}/vehicleTypes/getOne/${id}`, { 'headers': headers });
  }
  ...
  updateVehicleType(id: string | number, vehicleTypes:VehicleTypes) {
    return this.http.put(`${this.API_URI}/vehicleTypes/update/${id}`, vehicleTypes);
  }

  deleteVehicleType(token: any, id: string) {
    let headers = new HttpHeaders({ 'x-token': token });
    return this.http.delete(`${this.API_URI}/vehicleTypes/delete/${id}`, { 'headers': headers });
  }
}
