...
export class BlockTypesService {
  ...
  getBlockTypes(token: any) {
    ...
    return this.http.get(`${this.API_URI}/blockTypes/list`, {
      headers: headers,
    });
  }
  ...
  getBlockType(token: any, id: string) {
    let headers = new HttpHeaders({ 'x-token': token });
    return this.http.get(`${this.API_URI}/blockTypes/getOne/${id}`, {
      headers: headers,
    });
  }
  ...
  deleteBlockType(token: any, id: string) {
    let headers = new HttpHeaders({ 'x-token': token });
    return this.http.delete(`${this.API_URI}/blockTypes/delete/${id}`, {
      headers: headers,
    });
  }
}
