...
blockType:BlockTypes | any = {
  ...
  constructor(private blockTypeServices : BlockTypesService,  private recaptchaV3Service: ReCaptchaV3Service, config: NgbModalConfig, private modalService: NgbModal) { 
...
this.recaptchaV3Service.execute('action').subscribe(
  (token)=>{
    ...
    if (this.blockType.name_block_type == '' || this.blockType.block_status==0) {
...
this.recaptchaV3Service.execute('action').subscribe(
  ...
  this.blockTypeServices.updateBlockType(this.blockType.id_block_type,this.blockType).subscribe(
    (res: any) => {
      if (res['status']) {
        Swal.fire('¡Bloque!', res['message'], 'success');
        this.rerender();
