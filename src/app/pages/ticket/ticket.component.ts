...
export class TicketComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(private billService: BillsService, private ticketService: TicketService, private recaptchaV3Service: ReCaptchaV3Service, config: NgbModalConfig, private modalService: NgbModal) {
  ...
  saveTicket() {
    this.recaptchaV3Service.execute('action').subscribe(
      (token) => {
        ...
        if (this.ticket.pk_fk_id_block == 0 || this.ticket.fk_id_vehicle == 0 || this.ticket.entry_time == '') {
  ...
  updateTicket() {
    this.recaptchaV3Service.execute('action').subscribe(
      (token) => {
        ...
        this.ticketService.updateTicket(this.ticket.id_ticket, this.ticket).subscribe(
          (res: any) => {
            if (res['status']) {
              Swal.fire('Ticket!', res['message'], 'success');
              this.rerender();
