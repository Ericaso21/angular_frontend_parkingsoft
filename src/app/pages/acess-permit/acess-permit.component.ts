...
export class AcessPermitComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(private accessPermitService: AcessPermitsService, private userService: UserService, private recaptchaV3Service: ReCaptchaV3Service, config: NgbModalConfig, private modalService: NgbModal) {
  ...
  saveAccessPermit() {
    this.recaptchaV3Service.execute('action').subscribe(
      (token) => {
        if (this.accessPermit.fk_id_roles == 0 || this.accessPermit.fk_id_modules == 0) {
  ...
  updateAccessPermit() {
    this.recaptchaV3Service.execute('action').subscribe(
      (token) => {
        ...
        this.accessPermitService.updateAccessPermit(this.accessPermit.id_access_permits, this.accessPermit).subscribe(
          (res: any) => {
            if (res['status']) {
              Swal.fire('¡Permiso!', res['message'], 'success');
              this.rerender();
